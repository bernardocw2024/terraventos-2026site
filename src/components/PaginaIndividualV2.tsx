...
export default function PaginaIndividual({ slug, item: propItem }: PaginaIndividualProps) {
  ...
  const [loadingData, setLoadingData] = useState(!propItem);
  
  useEffect(() => {
    fetch('/properties-data.json').then(r => r.json()).then(d => { setRawData(d); setLoadingData(false); }).catch(() => setLoadingData(false));
  }, []);
  ...
  const resolvedItem = (() => {
    ...
    if (rawData.length > 0) {
      const rawItem = rawData.find(o => o.slug === currentSlug);
      if (rawItem) return resolveProperty(rawItem, i18n.language);
    }
    return null;
  })();
  
  if (loadingData || !resolvedItem) {
