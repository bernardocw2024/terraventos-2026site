...
type PaginaIndividualV2Props = {
  slug?: string;
  item?: OportunidadeDetalhe; // Permite passar o item via props se jÃÂ¡ tivermos, mas prioriza o slug
};

export default function PaginaIndividualV2({ slug, item: propItem }: PaginaIndividualV2Props) {
  const { t, i18n } = useTranslation();
  const [rawData, setRawData] = useState<any[]>([]);
  const [jsonLoaded, setJsonLoaded] = useState(!!propItem);

  useEffect(() => {
    fetch('/properties-data.json').then(r=>r.json()).then(d=>{setRawData(d);setJsonLoaded(true);}).catch(()=>setJsonLoaded(true));
  }, []);
  
  // Resolve o item com base no slug ou no propItem
  const resolvedItem: OportunidadeDetalhe | null = (() => {
    if (propItem && (!slug || propItem.slug === slug)) return propItem;
    const currentSlug = slug || window.location.pathname.split('/').pop() || '';
    if (rawData.length > 0) {
      const rawItem = rawData.find((o: any) => o.slug === currentSlug);
      if (rawItem) return resolveProperty(rawItem, i18n.language);
    }
    return null;
  })();
  ...
  // Coleta todas as fotos disponÃÂ­veis para o ÃÂ¡lbum
  ...
  return (
    <section className="pagina-individual">
      <div className="pi-main">
        ...
        {/* Ã¢ÂÂÃ¢ÂÂ GALLERY STRIP BUTTON Ã¢ÂÂÃ¢ÂÂ */}
        {allPhotos.length > 1 && (
            ...
            <div className="pi-facilities">
              {item.facilities.map((facilidadeOrig) => {
                ...
                const getIcon = (fac: string) => {
                  ...
                  if (l.includes('portaria') || l.includes('seguranÃÂ§a') || l.includes('security')) return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>;
                  ...
                  if (l.includes('projeto exclusivo') || l.includes('projeto urbanÃÂ­stico')) return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="12 2 15 8.5 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 9 8.5 12 2" /></svg>;
                  ...
                  if (l.includes('ÃÂ¡reas verdes') || l.includes('natureza')) return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 2L4 12H8L4 20H11V24H13V20H20L16 12H20L12 2Z" /></svg>;
            ...
            {item.videoSources && item.videoSources.length > 0 && (
              <div className="pi-video-gallery-v3">
                <div className="pi-section-header">
                  <h3 className="pi-section-title">{t('pagina.videoGallery') || 'Galeria de VÃÂ­deos'}</h3>
                </div>
        ...
        <div className="pi-map-wrapper">
          {item.mapUrl ? (
            <iframe
              ...
              title={`Mapa de localizaÃÂ§ÃÂ£o para ${item.propertyTitle}`}
            />
          ) : (
      ...
      {(lightboxImage || lightboxVideo) && createPortal(
        <div className="pi-lightbox" onClick={closeLightbox}>
          ...
          {lightboxImage && (
            <img 
              src={lightboxImage} 
              alt="VisualizaÃÂ§ÃÂ£o ampliada" 
